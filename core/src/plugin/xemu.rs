...
"#);
        ...
        println!("✅ xemu.toml généré: {:?}", toml_path);
        Ok(toml_path)
    }

    /// Génère la section [input.bindings] avec détection SDL2 des manettes
    fn generate_input_bindings() -> String {
        // Initialiser SDL2 pour détecter les manettes
                            ...
                            for i in 0..std::cmp::min(num_joysticks, 4) {
                                if controller_subsystem.is_game_controller(i) {
                                    if let Ok(controller) = controller_subsystem.open(i) {
                                        // Générer le GUID au format xemu (32 caractères hex)
                                        let guid = controller.instance_id();
                                        // xemu utilise un format GUID spécifique
                                        let guid_str = format!("{:032x}", guid as u128);
                                        bindings.push_str(&format!("    {{ gamepad_id = '{}'}}\n", guid_str));
                                        println!("🎮 Manette {} détectée: {}", i + 1, controller.name());
                                    }
                                }
                            }
    ...
    /// Scan récursif pour trouver MCPX, BIOS et HDD
...
impl XemuPlugin {
    ...
    /// Vérifie si l'ISO est un XISO valide, sinon le convertit
    fn ensure_xiso(&self, rom_path: &Path, progress: Option<&dyn Fn(String)>) -> Result<PathBuf> {
        if self.is_xiso(rom_path)? {
            if let Some(cb) = progress { cb("✅ Valid XISO detected".to_string()); }
            return Ok(rom_path.to_path_buf());
        }

        if let Some(cb) = progress { cb("⚠️ Redump ISO detected. Starting conversion...".to_string()); }
    ...
    fn is_xiso(&self, path: &Path) -> Result<bool> {
        ...
        // XISO commence par "MICROSOFT*XBOX*MEDIA" à 0x0
...
impl EmulatorPlugin for XemuPlugin {
    ...
    fn prepare_launch_config_with_progress(
        ...
        let final_rom_path = self.ensure_xiso(rom_path, progress).unwrap_or_else(|e| {
            eprintln!("⚠️ Warning: XISO check/conversion failed: {}. Using original ROM.", e);
    ...
    fn fullscreen_args(&self) -> Vec<String> {
        // Pour xemu, fullscreen doit être AVANT -dvd_path
        // On le gère dans prepare_launch_config ou via le TOML
        // Retourner vide pour éviter le mauvais ordre
        vec![]
    }
    ...
    /// Configure l'environnement xemu: copie BIOS + génère xemu.toml
    ...
    fn portable_launch_args(&self, fullscreen: bool) -> (Vec<String>, Vec<String>) {
        // -full-screen DOIT être AVANT -dvd_path
    ...
    fn validate_requirements(&self, source_path: Option<&Path>) -> Result<ValidationResult> {
        ...
        if mcpx_exists && bios_exists && hdd_exists && source_path.is_none() {
            ...
            return Ok(ValidationResult { 
                valid: true, 
                message: "✅ MCPX, BIOS et HDD déjà installés.".to_string(), 
                fixed: false 
            });
        }
        ...
        let src = match source_path {
            ...
            None => {
                ...
                if missing.is_empty() {
                    return Ok(ValidationResult { 
                        valid: true, 
                        message: "✅ Tous les fichiers sont présents.".to_string(), 
                        fixed: false 
                    });
                } else {
                    return Ok(ValidationResult { 
                        valid: false, 
                        message: format!("❌ Fichiers manquants: {}. Sélectionnez un dossier.", missing.join(", ")), 
                        fixed: false 
                    });
                }
            }
        ...
        let mcpx_installed = if let Some(ref mcpx) = found_mcpx {
            ...
            println!("✅ MCPX installé: {:?}", mcpx_path);
            true
        } else {
            mcpx_exists
        };
        ...
        let bios_installed = if let Some(ref bios) = found_bios {
            ...
            println!("✅ BIOS installé: {:?}", bios_path);
            true
        } else {
            bios_exists
        };
        ...
        if mcpx_installed { status.push("MCPX ✅"); } else { status.push("MCPX ❌"); }
        if bios_installed { status.push("BIOS ✅"); } else { status.push("BIOS ❌"); }
        if hdd_installed { status.push("HDD ✅"); } else { status.push("HDD ❌"); }

        let message = if all_installed {
            format!("✅ Installation complète! {} + xemu.toml généré", status.join(" | "))
        } else {
            format!("⚠️ Installation partielle: {}", status.join(" | "))
        };
