...
impl RyujinxPlugin {
                ...
                if let Ok(game_controller_subsystem) = sdl_context.game_controller() {
                    ...
                    for i in 0..available_joysticks {
                        if let Ok(controller) = game_controller_subsystem.open(i) {
                            ...
                            // CRITICAL: Ryujinx AUTHENTIC GUID generation (from SDL3GamepadDriver.cs)
                            let joystick = match sdl_context.joystick().unwrap().open(i) {
                                Ok(j) => j,
                                Err(_) => {
                                    println!("⚠️  Could not open joystick {} for GUID", i);
                                    continue;
                                }
                            };
                            ...
                            // Ryujinx SDLGuidToString rearrangement (line 68)
                            // C#: $"{strGuid[4..6]}{strGuid[6..8]}{strGuid[2..4]}{strGuid[0..2]}-{strGuid[10..12]}{strGuid[8..10]}-..."
                            ...
                            configs.push(InputConfig::StandardControllerInputConfig(StandardControllerInputConfig {
                                ...
                                rumble: RumbleConfig {
                                    strong_rumble: 1.0,
                                    weak_rumble: 1.0,
                                    enable_rumble: false,  // GUI default is false
                                },
                                led: LedConfig {
                                    enable_led: false,
                                    turn_off_led: false,
    ...
    fn update_ryujinx_input_config() -> Result<()> {
        ...
        if config_path.exists() {
            ...
            // LOG: Show what was written
            if let Some(input_configs) = json_config.get("input_config") {
                if let Some(first_config) = input_configs.get(0) {
                    let id = first_config.get("id").and_then(|v| v.as_str()).unwrap_or("N/A");
                    let name = first_config.get("name").and_then(|v| v.as_str()).unwrap_or("N/A");
                    println!("✅ Saved Controller Config → ID: {} | Name: {}", id, name);
...
pub struct StandardControllerInputConfig {
    ...
    // Ryujinx GUI Reference Order:
    // 1. Sticks
    // 2. Deadzones/Motion/Rumble/Led
    // 3. Buttons (Joycons)
    // 4. Metadata
